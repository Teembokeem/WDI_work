...
var notTrueStarkBoys = children.filter(function(child) {
  return trueName(child) !== "Stark" && child.sex === "M"
})
...
var anOn = children.filter(function(child) {
  return (/an|on/i.test(child.name))
})
...
var notStarks = children.filter(function(child) {
  return (!/Stark/i.test(child.house))
})
...
var dontFitIn = children.filter(function(child) {
  return trueName(child) !== "Stark" || (/arya/i.test(child.name))
})
...
children.forEach(function(child) {
  if (child.direWolf) dires.push(child.direWolf);
})
