...
function declaim(king) {
  if (!king.ironThrone) {
    return `Woe be to the pretender ${king.name} of the house ${king.house}…`;
  }
}

kings.forEach(function(king){
  if (!king.ironThrone) console.log(declaim(king));
});
...
function kneel(king) {
  return `The ${king.ironThrone ? "king" : "pretender"} ${king.name} of house ${king.house} must kneel!`;
}

console.log(kneel(kings[1]));
console.log(kneel(kings[0]));
