(function() {
  ...
  function TriumphsController($state, userDataService, $log, $http) {
    ...
    function getFishes() {
      $http.get('http://localhost:3000/api/fish').then(function(response) {
    ...
    function deleteFish(id) {
      $http.delete('http://localhost:3000/api/fish/' + id).then(function(response) {
    ...
    function postFish() {
      $http.post('http://localhost:3000/api/fish', vm.newFish)
        .then(getFishes)
    ...
    function updateFish(id) {
      $http.put('http://localhost:3000/api/fish/' + id, vm.editFish).then(function(response) {
        vm.editFish = {
          name: "",
          category: ""
        };
